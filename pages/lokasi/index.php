...
// Set page title
$page_title = "Data Lokasi Gudang - Stock Control Gudang";

// Include the header template
require_once '../../templates/header.php';
require_once '../../config/database.php';

$query = "SELECT * FROM lokasi_gudang ORDER BY id_lokasi DESC";
            ...
            <tbody>
                <?php if ($result->num_rows > 0): $no = 1; ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td><?= $no++; ?></td>
                            <td><strong><?= htmlspecialchars($row['kode_lokasi']); ?></strong></td>
                            <td><?= htmlspecialchars($row['nama_lokasi']); ?></td>
                            <td><?= $row['kapasitas'] ? number_format($row['kapasitas']) . ' unit' : 'Tidak terbatas'; ?></td>
                            <td><?= htmlspecialchars($row['deskripsi']) ?: '-'; ?></td>
                            <td>
                                <a href="edit.php?id=<?= $row['id_lokasi']; ?>" class="action-btn action-btn-edit">
                                    ✏️ Edit
                                </a>
                                <a href="hapus.php?id=<?= $row['id_lokasi']; ?>" 
