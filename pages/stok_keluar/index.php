...
// Set page title
$page_title = "Data Stok Keluar - Stock Control Gudang";
...
$query = "
SELECT sk.*, p.nama_produk, l.nama_lokasi, pl.nama_pelanggan 
FROM stok_keluar sk
...
<div class="container">
    ...
    <div class="nav-links">
        ...
        <a href="catat_stok.php" class="btn btn-primary">
            + Catat Stok Keluar
        </a>
    </div>
    ...
    <div class="glass-container">
        <table class="glass-table">
            ...
            <tbody>
                <?php if ($result->num_rows > 0): $no = 1; ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            ...
                            <td><?= number_format($row['jumlah_keluar']); ?> unit</td>
                            <td><?= htmlspecialchars($row['nama_pelanggan']) ?? '-'; ?></td>
                            <td>
                                <?php
                                $tipe = strtolower($row['tipe_keluar']);
                                $badgeClass = 'type-badge-lainnya';
                                if (strpos($tipe, 'penjualan') !== false) {
                                    $badgeClass = 'type-badge-penjualan';
                                } elseif (strpos($tipe, 'retur') !== false) {
                                    $badgeClass = 'type-badge-retur';
                                } elseif (strpos($tipe, 'rusak') !== false) {
                                    $badgeClass = 'type-badge-rusak';
                                }
                                ?>
                                <span class="type-badge <?= $badgeClass; ?>">
                                    <?= htmlspecialchars($row['tipe_keluar']); ?>
                                </span>
                            </td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="10" style="text-align: center; padding: 40px; color: var(--md-sys-color-on-surface-variant);">
